...
public class TenantContext
{
    ...
    /// <summary>
    /// HTTP Context'teki kullanıcıdan CompanyCode alır ve
    /// buna göre tenant (şirket) bilgisini set eder.
    /// </summary>
    private void InitializeTenant()
    {
        ...
        // Kullanıcının claim'lerinden "CompanyCode" değerini al
        var companyCode = user.FindFirst("CompanyCode")?.Value;
        if (string.IsNullOrEmpty(companyCode))
            return;

        // Şirket bilgisini ManagementDB'den çek
